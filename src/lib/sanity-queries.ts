export const GROQ_ORDER_MANUAL_SORT_THEN_NEWEST =
  "coalesce(sortOrder, 99) asc, _createdAt desc";
...
export const PRODUCTS_SEARCH_INDEX_QUERY = `*[_type == "product" && defined(slug.current)] {
    _id,
    title,
    brand,
    "slug": slug.current,
...
export const PRODUCT_BY_SLUG_QUERY = `*[_type == "product" && slug.current == $slug][0]{
  ...
  "categorySlug": coalesce(
    (categories[]-> | order(${GROQ_ORDER_MANUAL_SORT_THEN_NEWEST}))[0].slug.current
  ),
  origin,
  stock,
  description,
  "image": coalesce(
    select(defined(image.asset._ref) => image),
    images[defined(asset._ref)][0]
  ),
