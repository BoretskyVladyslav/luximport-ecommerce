...
export const IMPORT_COLUMN_SPEC = [
  ...
  { field: "barcode", headers: ["Штрихкод", "barcode"] },
  {
    ...
    headers: ["оптова (+1 merged)", "wholesalePrice"],
  },
...
export function parseProductWorkbook(
  ...
  if (!detected) {
    throw new Error(
      "Could not detect header row (need Артикул/SKU or Назва/title)",
    );
  }
  ...
  for (let i = headerRowIndex + 1; i < rows.length; i++) {
    ...
    const price =
      columns.price !== undefined
        ? toNumber(readCell(row, columns.price))
        : undefined;
    const wholesalePrice =
      columns.wholesalePrice !== undefined
        ? toNumber(readCell(row, columns.wholesalePrice))
        : undefined;
    const stockRaw =
      columns.stock !== undefined
        ? toNumber(readCell(row, columns.stock))
        : undefined;
    const piecesPerBox =
      columns.piecesPerBox !== undefined
        ? toNumber(readCell(row, columns.piecesPerBox))
        : undefined;

    if (!title) errors.push("missing title");
    if (!sku) errors.push("missing sku");
    if (price !== undefined && price <= 0) errors.push("price must be > 0");
    if (stockRaw !== undefined && stockRaw < 0)
      errors.push("stock cannot be negative");
    if (
      piecesPerBox !== undefined &&
      (!Number.isInteger(piecesPerBox) || piecesPerBox < 1)
    ) {
    ...
    parsed.push({
      ...
      description:
        cellToString(readCell(row, columns.description)) || undefined,
      category: cellToString(readCell(row, columns.category)) || undefined,
      subcategory:
        cellToString(readCell(row, columns.subcategory)) || undefined,
    ...
    const ext = contentType.includes("png")
      ...
      : contentType.includes("webp")
        ...
        : contentType.includes("gif")
          ...
          : "jpg";
    const base =
      url.pathname.split("/").filter(Boolean).pop() || `product.${ext}`;
...
async function resolveCategoryRef(
  ...
  return (
    pick(subcategoryTitle, "subcategory") ?? pick(categoryTitle, "category")
  );
}
...
export async function applyProductImport(
  ...
  const catalog = await client.fetch<
    Array<{ _id: string; _type: string; title?: string | null }>
  ...
  for (const row of parsed) {
    ...
    const existingId =
      skuMap.get(sku) ??
      (row.barcode ? barcodeMap.get(row.barcode.trim()) : undefined);
    ...
    if (row.imageUrl && !options.dryRun) {
      ...
      if (!imageAssetId)
        imageNote = "image URL skipped (fetch failed or blocked)";
    }
    ...
    if (!existingId) {
      setFields.slug = {
        _type: "slug",
        current: uniqueSlug(slugifyTitleForSlug(title), sku, usedSlugs),
      };
    }
    ...
    if (row.wholesalePrice !== undefined)
      setFields.wholesalePrice = row.wholesalePrice;
    ...
    if (row.piecesPerBox !== undefined)
      setFields.piecesPerBox = row.piecesPerBox;
    if (row.description)
      setFields.description = textToPortableText(row.description);
    if (categoryId) {
      setFields.categories = [
        {
          _type: "reference",
          _ref: categoryId,
          _key: `cat-${categoryId}`.slice(0, 64),
        },
      ];
    }
    if (imageAssetId) {
