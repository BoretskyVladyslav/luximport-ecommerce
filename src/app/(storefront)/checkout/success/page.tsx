...
export default function CheckoutSuccessPage() {
  ...
  const [resolvedPaymentState, setResolvedPaymentState] = useState<
  ...
  useEffect(() => {
    ...
    try {
      expectFlag =
        sessionStorage.getItem("luximport_checkout_expect_success") === "1";
    } catch {
      expectFlag = false;
    }
  ...
  useEffect(() => {
    ...
    const runCheck = async () => {
      const res = await fetch(
        `/api/orders/by-reference?order=${encodeURIComponent(orderFromUrl)}`,
        {
          method: "GET",
          cache: "no-store",
        },
  ...
  useEffect(() => {
    if (!isHydrated) return;
    void revalidateProfilePath().catch(() => {});
  }, [isHydrated]);
      ...
      <div className={styles.shell}>
        ...
        >
          ...
          <p className={styles.fallbackSub}>
            Перейдіть на головну або до каталогу.
          </p>
    ...
    return (
      <div className={styles.shell}>
        ...
        >
          ...
          <p className={styles.fallbackSub}>
            Спробуйте оплатити замовлення повторно в кабінеті.
          </p>
