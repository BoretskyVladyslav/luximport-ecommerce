...
import {
  normalizeMerchantDomainName,
  WAYFORPAY_GOOGLE_PAY,
} from "@/lib/wayforpay-purchase";
...
function mapDetailLinesFromApi(raw: unknown): ProfileOrderLine[] {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    const productId =
      typeof r.productId === "string" ? r.productId : `line-${i}`;
    const title = typeof r.title === "string" ? r.title : "";
    const quantity =
      typeof r.quantity === "number" && Number.isFinite(r.quantity)
        ? Math.max(0, Math.trunc(r.quantity))
        : 0;
    const price =
      typeof r.price === "number" && Number.isFinite(r.price) ? r.price : 0;
    const image =
      r.image !== undefined && r.image !== null ? r.image : undefined;
...
function mapApiOrderRow(o: Record<string, unknown>): OrderListEntry | null {
  ...
  const date =
    dt && !Number.isNaN(dt.valueOf()) ? dt.toLocaleDateString("uk-UA") : "";
  ...
  const totalAmount =
    typeof o.totalAmount === "number" && Number.isFinite(o.totalAmount)
      ? o.totalAmount
      : 0;
  ...
  const trackingNumber =
    typeof o.trackingNumber === "string" && o.trackingNumber.trim()
  ...
  return {
    ...
    shippingAddress:
      typeof o.shippingAddress === "string" ? o.shippingAddress : "",
...
function buildDetailLines(order: OrderListEntry): ProfileOrderLine[] {
  if (order.detailLines && order.detailLines.length > 0)
    return order.detailLines;
...
function paymentForDisplay(order: OrderListEntry): OrderPaymentStatus {
  ...
  if (p === "pending" || p === "paid" || p === "cancelled" || p === "failed")
    return p;
  return "pending";
}
...
function itemsCountForOrder(order: OrderListEntry): number {
  if (
    ...
    Number.isFinite(order.itemsCount)
  ) {
...
export default function ProfilePage() {
  ...
  const { user, isAuthenticated, refresh, updateUser, destroySession } =
    useUser();
  const {
    data: session,
    ...
    async function run() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setValue(
            "firstName",
            nextUser.firstName || (nextUser.name || "").split(" ")[0] || "",
            { shouldValidate: false },
          );
          setValue(
            "lastName",
            nextUser.lastName ||
              (nextUser.name || "").split(" ").slice(1).join(" ") ||
              "",
            { shouldValidate: false },
          );
          setValue("phone", nextUser.phone || "", { shouldValidate: false });
          setValue("address", nextUser.address || "", {
            shouldValidate: false,
          });
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    void fetchServerOrders()
      .then((mapped) => {
        ...
        const hasPending = mapped.some(
          (o) => (o.payment ?? "pending") === "pending",
        );
          ...
          const timer = window.setInterval(() => {
            ...
            void fetchServerOrders()
              .then((next) => {
                ...
                if (
                  next.every((o) => (o.payment ?? "pending") !== "pending") ||
                  attempts >= 5
                ) {
                  window.clearInterval(timer);
                }
  ...
  const startProfilePayment = useCallback(
    async (sanityDocumentId: string) => {
      ...
      if (!wayforpayScriptReady || !window.Wayforpay) {
        toast.error(
          "Платіжна форма ще завантажується. Спробуйте за кілька секунд.",
        );
        return;
      }
      if (!user) return;
      ...
      try {
        ...
        const merchantAccount =
          typeof details.merchantAccount === "string"
            ? details.merchantAccount.trim()
            : "";
        const rawDomain =
        ...
        wfp.run(
          {
            ...
            currency:
              typeof details.currency === "string" ? details.currency : "UAH",
            productName,
            productPrice,
            productCount,
            ...
            googlePay:
              typeof details.googlePay === "string" && details.googlePay.trim()
                ...
                : WAYFORPAY_GOOGLE_PAY,
            ...(typeof details.returnUrl === "string"
              ? { returnUrl: details.returnUrl }
              : {}),
            ...(typeof details.serviceUrl === "string"
          ...
          () => {
            void fetch("/api/orders/sync-payment", {
              ...
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                orderId: sanityDocumentId,
                transactionReference: String(data.retryReference),
              }),
            })
              .then((res) => {
                if (!res.ok) {
          ...
          <div className="animate-pulse">
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="rounded-lg border border-stone-200 bg-white px-6 py-5"
                >
  ...
  const onSaveSettings = async (values: ProfileUpdateFormData) => {
    ...
    try {
      ...
      updateUser({
        ...
        phone:
          typeof u.phone === "string" ? u.phone : (values.phone?.trim() ?? ""),
        address:
      ...
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          <div className={styles.tabNavigation}>
            ...
            <button className={styles.tabBtn} onClick={handleLogout}>
              ВИЙТИ З АКАУНТА
            </button>
          </div>
