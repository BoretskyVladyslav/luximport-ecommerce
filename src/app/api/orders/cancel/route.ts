...
export async function POST(req: Request) {
  try {
    ...
    if (typeof orderIdRaw !== "string" || !orderIdRaw.trim()) {
      return NextResponse.json(
        { message: "Некоректний ідентифікатор замовлення." },
        { status: 400 },
      );
    }
    ...
    const doc = (await writeClient.getDocument(orderId)) as Record<
      string,
      unknown
    > | null;
    ...
    const payment =
      typeof doc.paymentStatus === "string" ? doc.paymentStatus : "pending";
    ...
    if (payment !== "pending") {
      return NextResponse.json(
        { message: "Скасування недоступне для цього замовлення." },
        { status: 409 },
      );
    }

    const rawItems = doc.items;
    const lines: OrderLine[] = Array.isArray(rawItems)
      ? (rawItems as OrderLine[])
      : [];
    ...
    const stockRows =
      restoreIds.length > 0
        ? await writeClient.fetch<Array<{ _id: string; stock: number | null }>>(
            `*[_type == "product" && _id in $ids && !(_id match "drafts.*")]{ _id, stock }`,
            { ids: restoreIds },
          )
        : [];
    const trackedIds = new Set(
      stockRows
        .filter(
          (row) => typeof row.stock === "number" && Number.isFinite(row.stock),
        )
        .map((row) => row._id),
    );
    ...
    try {
        ...
        lines.map((item) => {
          ...
          const qty =
            typeof item.quantity === "number" && Number.isFinite(item.quantity)
              ...
              : 0;
          if (!pid || qty <= 0 || !trackedIds.has(pid)) {
      ...
      return NextResponse.json(
        { message: "Не вдалося повернути товари на склад." },
        { status: 500 },
      );
    }
