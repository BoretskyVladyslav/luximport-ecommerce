...
export async function POST(req: Request) {
  ...
  try {
    ...
    const createOrderRes = await fetch(
      new URL("/api/checkout/create-order", req.url),
      {
        method: "POST",
        headers: {
    ...
    const orderReference =
      typeof createdOrder?.sanityDocumentId === "string"
        ? createdOrder.sanityDocumentId
        : "";
    if (!orderReference) {
      return NextResponse.json(
        {
          code: "CHECKOUT_CREATE_ORDER_FAILED",
    ...
    const paymentRes = await fetch(new URL("/api/payment/init", req.url), {
      ...
      body: JSON.stringify({
        ...
        productName: Array.isArray(b.items)
          ? b.items.map((i: any) => String(i?.title || "Товар"))
          : [],
        ...
        productPrice: Array.isArray(b.items)
          ? b.items.map((i: any) => {
              const price = unitPriceForQuantity({
                price: typeof i?.price === "number" ? i.price : 0,
                wholesalePrice:
                  typeof i?.wholesalePrice === "number"
                    ? i.wholesalePrice
                    : null,
                wholesaleMinQuantity:
                  typeof i?.wholesaleMinQuantity === "number"
                    ? i.wholesaleMinQuantity
                    : null,
                piecesPerBox:
                  typeof i?.piecesPerBox === "number" ? i.piecesPerBox : null,
    ...
    return NextResponse.json(
      {
        ...
        message: "Внутрішня помилка checkout-сесії.",
        correlationId,
      },
      { status: 500 },
    );
