...
const SUBCATEGORY_SLUG_HELP =
  "Натисніть «Generate», щоб створити посилання з назви. На сайті: ваш-домен/catalog?category=ваш-slug. У межах однієї головної категорії не можна повторювати slug підкатегорій. Після публікації змінюйте лише за потреби.";
...
function subcategorySlugUnique() {
  return async (
    ...
    if (!parentRef) {
      ...
      return count === 0
        ? true
        : "Спочатку оберіть головну категорію; цей URL уже зайнятий";
    }
    ...
    return count === 0
      ? true
      : "Цей URL уже використовується в цій головній категорії";
  };
}
