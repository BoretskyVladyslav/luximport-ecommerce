 ...
 *   "Залишок"             → stock            (number)  only set when the cell has a number
 *
 * Usage:
...
if (!NEXT_PUBLIC_SANITY_PROJECT_ID || !SANITY_API_TOKEN) {
  console.error(
    "❌  Missing required env vars: NEXT_PUBLIC_SANITY_PROJECT_ID and/or SANITY_API_TOKEN",
  );
  process.exit(1);
}
...
const COLUMNS = {
  ...
  mainGroup: {
    labels: ["головна ГРУПА", "Головна ГРУПА", "ГОЛОВНА ГРУПА"],
    offset: 0,
  },
  ...
  stock: { labels: ["Залишок", "Залишок на складі", "stock"], offset: 0 },
};
  ...
  for (let i = 0; i < Math.min(rows.length, 30); i++) {
    ...
    for (const [field, { labels, offset }] of Object.entries(COLUMNS)) {
      const idx = row.findIndex((c) =>
        labels.some(
          (l) => c?.toString().trim().toLowerCase() === l.toLowerCase(),
        ),
      );
      if (idx === -1) {
        console.warn(
          `   ⚠️  Column not found for "${field}" (searched: ${labels.join(", ")})`,
        );
      } else {
        colMap[field] = idx + offset;
        console.log(
          `   ✔ "${field}" → col ${colMap[field]}  ["${row[idx]}"${offset ? ` +${offset}` : ""}]`,
        );
      }
    }
...
async function getOrCreateCategory(title, parentId = null) {
  ...
  // Create new category
  console.log(
    `   📁 Creating category: "${title}"${parentId ? ` (parent: ${parentId})` : ""}`,
  );
  if (DRY_RUN) {
...
function standardizeCategory(rawName) {
  ...
  if (clean.includes("кондитер") || clean.includes("кондитор"))
    return "Кондитерські вироби";
  if (clean.includes("гарячі") || clean.includes("напої"))
    return "Гарячі напої";
...
async function run() {
  ...
  if (!detected) {
    console.error(
      '❌  Could not find header row containing "Артикул". Aborting.',
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    let rawMain =
      colMap.mainGroup !== undefined
        ? standardizeCategory(row[colMap.mainGroup])
        : "";
    let rawSub =
      colMap.subGroup !== undefined
        ? standardizeCategory(row[colMap.subGroup])
        : "";
    ...
    const sku =
      colMap.sku !== undefined ? row[colMap.sku]?.toString().trim() : null;
    ...
    const piecesPerBox =
      colMap.piecesPerBox !== undefined
        ? toNumber(row[colMap.piecesPerBox])
        : null;
    const price =
      colMap.price !== undefined ? toNumber(row[colMap.price]) : null;
    const wholesalePrice =
      colMap.wholesalePrice !== undefined
        ? toNumber(row[colMap.wholesalePrice])
        : null;
    const stockRaw =
      colMap.stock !== undefined ? toNumber(row[colMap.stock]) : null;
    const stock =
      stockRaw !== null && stockRaw >= 0 ? Math.trunc(stockRaw) : null;
  ...
  try {
    ...
    console.warn(
      `⚠️  Could not pre-fetch products: ${err.message}. Will query per-row.`,
    );
  }
  ...
  console.log(
    `📋  ${toCreate.length} to CREATE  |  ${toUpdate.length} to PATCH\n`,
  );
  ...
  if (toUpdate.length > 0) {
    ...
    for (const p of toUpdate) {
      ...
      const setPayload = {
        ...
        ...(p.categoryId && {
          categories: [
            { _type: "reference", _ref: p.categoryId, _key: p.categoryId },
          ],
        }),
      };

      // Empty Excel cells must not wipe price, wholesale, box qty, or stock.
      const unsetPayload = [...(p.weight == null ? ["weight"] : [])];
      ...
      try {
        ...
        console.log(
          `   ✏️  Row ${p.rowNumber}: patched ${docId}  (SKU: ${p.sku})`,
        );
        updatedCount++;
      } catch (err) {
        console.error(
          `   ❌  Row ${p.rowNumber}: patch failed (${p.sku}): ${err.message}`,
        );
        errorCount++;
      }
    }
    ...
    for (const p of toCreate) {
      ...
      try {
        if (!DRY_RUN) {
          ...
          console.log(
            `   ➕  Row ${p.rowNumber}: created ${created._id}  (SKU: ${p.sku})`,
          );
        } else {
          console.log(
            `   ➕  Row ${p.rowNumber}: [DRY RUN] would create  (SKU: ${p.sku})`,
          );
        }
        createdCount++;
      } catch (err) {
        console.error(
          `   ❌  Row ${p.rowNumber}: create failed (${p.sku}): ${err.message}`,
        );
        errorCount++;
      }
    }
