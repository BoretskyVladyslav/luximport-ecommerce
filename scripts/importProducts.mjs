...
if (!NEXT_PUBLIC_SANITY_PROJECT_ID || !SANITY_API_TOKEN) {
  console.error(
    "❌  Missing required Sanity env vars. Check NEXT_PUBLIC_SANITY_PROJECT_ID and SANITY_API_TOKEN in .env.local",
  );
  process.exit(1);
}
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (const [field, { variants, offset }] of Object.entries(HEADER_MAP)) {
      const idx = row.findIndex((cell) =>
        variants.some(
          (v) => cell?.toString().trim().toLowerCase() === v.toLowerCase(),
        ),
      );
      if (idx === -1) {
        console.warn(
          `   ⚠️  Header not found for "${field}" (looked for: ${variants.join(", ")}). Skipped.`,
        );
      } else {
...
async function run() {
  ...
  if (rows.length < 2) {
    console.error(
      "❌  The worksheet appears to be empty or has only a header row.",
    );
    process.exit(1);
  }
  ...
  if (!detected) {
    console.error(
      '❌  Could not find the header row (no row containing "Артикул"). Aborting.',
    );
    process.exit(1);
  }
  ...
  if (COL.title === undefined) {
    console.error(
      '❌  Could not find the title column ("Повне найменування"). Aborting.',
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const wPriceRaw =
      COL.wholesalePrice !== undefined ? row[COL.wholesalePrice] : undefined;
    const pcsRaw =
      COL.piecesPerBox !== undefined ? row[COL.piecesPerBox] : undefined;
  ...
  try {
    ...
    console.warn(
      `⚠️  Could not fetch existing products: ${err.message}. Will create all as new.`,
    );
  }
  ...
  if (toCreate.length > 0) {
    ...
    console.log(
      `🚀  Creating ${toCreate.length} new product(s) in ${totalBatches} batch(es)…\n`,
    );
    for (let b = 0; b < totalBatches; b++) {
      const start = b * BATCH_SIZE;
      const chunk = toCreate.slice(
        start,
        Math.min(start + BATCH_SIZE, toCreate.length),
      );
      console.log(
        `   Batch ${b + 1}/${totalBatches}: rows ${chunk[0].rowNumber}–${chunk[chunk.length - 1].rowNumber}`,
      );
      ...
      try {
        ...
        for (const { doc, rowNumber } of chunk) {
          try {
            ...
            console.log(`      ✔ Row ${rowNumber}: ${r._id}`);
            createdCount++;
          } catch (e) {
            console.error(`      ✘ Row ${rowNumber}: ${e.message}`);
            errorCount++;
          }
        }
      }
  ...
  if (toUpdate.length > 0) {
    ...
    for (const { existingId, doc, rowNumber } of toUpdate) {
      ...
      const unsetFields = [...(!doc.weight ? ["weight"] : [])];
      try {
        const patch = client.patch(existingId).set(setFields);
        if (unsetFields.length) patch.unset(unsetFields);
        await patch.commit();
        console.log(
          `   ✏️  Row ${rowNumber}: patched ${existingId} (SKU ${doc.sku})`,
        );
        updatedCount++;
      } catch (err) {
        console.error(
          `   ❌  Row ${rowNumber}: patch failed for ${existingId}: ${err.message}`,
        );
        errorCount++;
      }
    }
